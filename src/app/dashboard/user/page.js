...
import {
  ...
  Navigation,
  Trophy,
  Recycle, // Icon Navigasi GPS + Trophy + Recycle
} from "lucide-react";
...
const DRIVER_START = [107.625, -6.925];
...
export default function UserDashboard() {
    ...
    if (
      ...
      const interval = setInterval(() => {
        setDriverLocation((prev) => {
          ...
          // Jika dekat, stop
          if (Math.abs(lonDiff) < 0.00001 && Math.abs(latDiff) < 0.00001)
            return prev;
    ...
    try {
      ...
      if (profileData?.rt) {
        const { data: notifData } = await supabase
          .from("notifications")
          .select("*")
          .or(`rt.eq.${profileData.rt},rt.is.null`)
      ...
      const { data: transactions } = await supabase
        .from("transactions")
        .select(
          `*, waste_types(name, price_per_kg), drivers:driver_id(full_name)`,
        )
      ...
      if (transactions) {
        ...
        const totalW = transactions.reduce(
          (acc, curr) => acc + (Number(curr.weight) || 0),
          0,
        );
        setStats({
          totalWaste: totalW,
          lastPickup: transactions[0]
            ? new Date(transactions[0].created_at).toLocaleDateString("id-ID")
            : "-",
          points: Math.floor(totalW * 10),
        });

        const active = transactions.find(
          (t) => t.status === "Pending" || t.status === "Process",
        );
        if (active) {
          setActivePickup({
            id: active.id,
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      Swal.fire("Sukses", "Profil disimpan", "success");
      fetchData();
    } catch (e) {}
  };
  ...
  const handleRequestSubmit = async (e) => {
    ...
    if (!formRequest.wasteTypeId) {
      Swal.fire("Gagal", "Pilih sampah", "error");
      return;
    }
  ...
  const handleComplaintSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire("Terkirim", "Aduan dikirim", "success");
      setComplaintForm({ title: "", category: "", content: "" });
      fetchData();
    } catch (e) {}
  };

  const handlePayClick = (b) =>
    setPaymentModal({ open: true, bill: b, step: "method" });
  ...
  const processPayment = async () => {
    ...
    if (!window.snap) {
      Swal.fire(
        "Error",
        "Sistem pembayaran sedang memuat, mohon tunggu.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      window.snap.pay(data.token, {
        onSuccess: async function (result) {
          ...
          setBills((prev) =>
            prev.map((b) =>
              b.id === currentBill.id ? { ...b, status: "Paid" } : b,
            ),
          );
  ...
  const getStatusColor = (s) =>
    s === "Done" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700";
  ...
  const renderDashboard = () => (
    <div className="space-y-6 animate-in fade-in text-gray-800 pb-20 md:pb-0">
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl shadow-lg p-6 md:p-8 text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="opacity-90">
            {profile?.rt
              ? `Warga RT ${profile.rt} / RW ${profile.rw}`
              : "Lengkapi data RT/RW di profil."}
          </p>
          <p className="text-xs mt-2 opacity-75">
            Kumpulkan poin untuk naik level! ({stats.points} poin)
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {[
          { l: "Total Sampah", v: `${stats.totalWaste} kg`, i: Trash2 },
          { l: "Poin", v: stats.points, i: Award },
          { l: "Pickup Terakhir", v: stats.lastPickup, i: Calendar },
        ].map((s, i) => (
          <div
            key={i}
      ...
      <form onSubmit={handleRequestSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormRequest({ ...formRequest, wasteTypeId: e.target.value })
              }
              required
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormRequest({ ...formRequest, weight: e.target.value })
              }
              required
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormRequest({ ...formRequest, date: e.target.value })
              }
              required
            />
          </div>
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormRequest({ ...formRequest, time: e.target.value })
              }
              required
            >
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setFormRequest({ ...formRequest, notes: e.target.value })
            }
          ></textarea>
        </div>
        <button
        ...
        <form onSubmit={handleComplaintSubmit} className="space-y-4 mb-8">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setComplaintForm({ ...complaintForm, content: e.target.value })
              }
              required
            ></textarea>
          </div>
          ...
          {complaints.length > 0 ? (
            complaints.map((c) => (
              ...
              >
                ...
                <p className="text-sm text-gray-700 bg-gray-50 p-2 rounded mb-2 italic">
                  "{c.content}"
                </p>
                {c.response && (
                  <div className="mt-2 pl-3 border-l-4 border-green-500">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-right pb-20 md:pb-0">
      <div className="lg:col-span-2 bg-gray-200 rounded-xl h-64 md:h-[450px] relative overflow-hidden border shadow-inner z-0">
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur px-6 py-2 rounded-full shadow-lg flex items-center gap-2 animate-bounce w-max border border-gray-200">
          ...
          <div>
            <p className="text-xs font-bold text-gray-500">Estimasi</p>
            <p className="font-bold text-green-700 text-sm">
              {activePickup?.eta || "Menunggu"}
            </p>
          </div>
        </div>
      </div>
        ...
        {activePickup ? (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-yellow-50 text-yellow-800 rounded text-sm border border-yellow-200">
              Mohon bersiap, driver sedang menuju titik lokasi Anda.
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button className="py-2 bg-green-50 text-green-700 rounded font-bold text-sm flex justify-center gap-1 hover:bg-green-100 transition">
                <MessageCircle size={16} /> Chat
              </button>
              <button className="py-2 bg-blue-50 text-blue-700 rounded font-bold text-sm flex justify-center gap-1 hover:bg-blue-100 transition">
                <Phone size={16} /> Call
              </button>
            </div>
          </div>
    ...
    <div className="space-y-6 animate-in slide-in-from-right pb-20 md:pb-0">
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800">Tagihan Iuran</h3>
            <p className="text-sm text-gray-500">
              Kelola pembayaran bulanan Anda.
            </p>
          </div>
        </div>
        {bills.map((bill) => (
          ...
          >
            ...
            {bill.status === "Unpaid" ? (
              ...
              <span className="w-full md:w-auto text-green-600 font-bold flex items-center justify-center md:justify-end gap-1">
                <CheckCheck size={16} /> Lunas
              </span>
            )}
          </div>
        ))}
  ...
  const renderEdukasi = () => (
    <div className="space-y-6 animate-in slide-in-from-right pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="flex items-center gap-2 w-full md:w-64 bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {EDUCATION_DATA.filter(
          (i) =>
            (selectedCategory === "Semua" || i.category === selectedCategory) &&
          ...
          >
            <div className="h-40 bg-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-green-50 transition">
              {item.type === "video" ? (
                <PlayCircle size={40} className="group-hover:text-green-600" />
              ) : (
                <BookOpen size={40} className="group-hover:text-green-600" />
              )}
            </div>
            <div className="p-5">
              ...
              <h3 className="font-bold text-lg mt-2 mb-2 text-gray-800 group-hover:text-green-600">
                {item.title}
              </h3>
              <p className="text-sm text-gray-500 mb-4 line-clamp-2">
                {item.desc}
              </p>
              <button className="text-green-600 font-bold text-sm flex items-center gap-1">
  ...
  const renderNotifikasi = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-in slide-in-from-right pb-20 md:pb-0">
      <div className="flex justify-between mb-4">
        <h3 className="font-bold text-xl text-gray-800">Notifikasi</h3>
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
          ...
          <div className="p-4 bg-green-50 rounded-lg border border-green-100">
            ...
            <p className="font-medium text-lg text-green-900">
              {profile?.rt ? (
                ...
                <span className="text-red-500 text-sm italic">Belum diisi</span>
              )}
            </p>
          </div>
        </div>
          ...
          pickupHistory.map((pickup, idx) => (
            ...
            >
              <div className="flex gap-4 items-center">
                ...
                <div>
                  <p className="font-bold text-gray-800">{pickup.type}</p>
                  <p className="text-sm text-gray-500">
  ...
  if (loading)
    return (
      <div className="min-h-screen flex justify-center items-center text-gray-800 font-bold">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans text-gray-800 overflow-x-hidden">
      ...
      >
        <div className="md:hidden flex justify-between items-center mb-6 sticky top-0 bg-gray-50 z-30 py-2">
          <button onClick={() => setIsMobileMenuOpen(true)}>
            <Menu size={28} />
          </button>
          <span className="font-bold text-lg capitalize">{activePage}</span>
            ...
            <div className="flex items-center gap-3 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
              ...
              >
                ...
                <div className="text-left hidden md:block">
                  <p className="text-xs font-bold text-gray-800 line-clamp-1 max-w-[100px]">
                    {profile?.full_name}
                  </p>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between mb-4 border-b pb-2">
              <h3 className="font-bold text-lg text-gray-800">Detail</h3>
              <button
                onClick={() => {
            ...
            {selectedHistory && (
              <div className="space-y-2 text-gray-800">
                ...
                <p>Status: {selectedHistory.status}</p>
                <p className="font-bold text-green-600">
                  Total: Rp {selectedHistory.fee.toLocaleString()}
                </p>
              </div>
            )}
            {selectedArticle && (
              <div className="space-y-2 text-gray-800">
                <h4 className="font-bold">{selectedArticle.title}</h4>
                <p className="text-sm">{selectedArticle.desc}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {paymentModal.open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6 text-center shadow-xl">
            <div className="flex justify-between mb-4">
              <h3 className="font-bold text-lg text-gray-800">Pembayaran</h3>
              <button
                onClick={() =>
            ...
            {paymentModal.step === "method" && (
              <div>
                <h2 className="text-3xl font-bold mb-4 text-gray-800">
                  Rp {paymentModal.bill.amount.toLocaleString()}
                </h2>
                <button
            ...
            {paymentModal.step === "success" && (
              <div>
                <CheckCheck className="mx-auto mb-2 text-green-600" size={40} />
                <h3 className="font-bold text-xl text-gray-800">Berhasil!</h3>
              </div>
            )}
          </div>
        </div>
