...
export default function DriverDashboard() {
  ...
  const centerToMyLocation = () => {
    if (mapRef.current && isGpsActive) {
      mapRef.current
        .getView()
        .animate({ center: fromLonLat(myLocation), zoom: 17, duration: 1000 });
    } else {
      Swal.fire(
        "GPS Mencari...",
        "Pastikan izin lokasi aktif dan tunggu sinyal.",
        "info",
      );
    }
  };
  ...
  const updateMapMarkers = () => {
    ...
    if (isGpsActive) {
      ...
      const accuracyFeature = new Feature({
        geometry: new Point(fromLonLat(myLocation)),
      });
    ...
    tasks
      .filter(
        (t) =>
          t.status === "Pending" ||
          (t.status === "Process" && t.driver_id === currentUser?.id),
      )
      .forEach((t) => {
        if (t.lng && t.lat) {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: activeTasks } = await supabase
        .from("transactions")
        .select(
          `*, profiles(full_name, address, rt, rw, phone), waste_types(name, price_per_kg)`,
        )
        .neq("status", "Done")
      ...
      const filteredTasks = (activeTasks || []).filter((t) => {
        ...
        if (t.status === "Pending") {
          ...
          return myAreas.some(
            (area) => area.rw == userRW && (area.rt == userRT || !area.rt), // !area.rt berarti mencakup satu RW full
          );
        }
      ...
      const income = (historyData || []).reduce(
  ...
  const contactUser = (type, phoneNumber, name) => {
    if (!phoneNumber)
      return Swal.fire("Gagal", "Nomor HP tidak tersedia.", "error");
  ...
  const handleTaskAction = async (taskId, action) => {
    const myActiveTask = tasks.find(
      (t) => t.status === "Process" && t.driver_id === currentUser.id,
    );

    // A. Jika Action = Process (Ambil Tugas)
    if (action === "Process") {
      if (myActiveTask)
        return Swal.fire("Fokus!", "Selesaikan tugas aktif dulu.", "warning");
      ...
      if (result.isConfirmed) {
        try {
          await supabase
            .from("transactions")
            .update({ status: "Process", driver_id: currentUser.id })
            .eq("id", taskId);
      ...
      if (finalWeight) {
        ...
        if (confirmResult.isConfirmed) {
          try {
            ...
            Swal.fire(
              "Tugas Selesai!",
              `Saldo Rp ${totalPrice.toLocaleString()} dicatat.`,
              "success",
            );
            fetchData();
          } catch (err) {
  ...
  const handleDriverStatusToggle = async () => {
    ...
    const newStatus =
      driverProfile.status === "On Duty" ? "Off Duty" : "On Duty";
    try {
      await supabase
        .from("drivers")
      ...
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-6 md:p-8 text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="opacity-90">
            Siap menjemput rezeki dan menjaga lingkungan hari ini?
          </p>
        ...
        .map((task) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
              <div>
                <h3 className="font-bold text-xl text-gray-800 flex items-center gap-2">
                  <Truck className="text-blue-600" /> Sedang Dijemput
                </h3>
                <p className="text-sm text-gray-500 mt-1">
                  Order ID: #{String(task.id).slice(0, 8)}
                </p>
              </div>
              <span className="bg-blue-100 text-blue-700 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm">
                PROCESS
              </span>
            </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 font-medium">
                      ...
                      <span className="font-bold">{task.weight} Kg</span>
                    </p>
                    <p className="text-sm text-green-600 font-bold mt-1">
                      Rp {task.total_price?.toLocaleString()}
                    </p>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100">
              ...
              >
                <CheckCircle size={20} /> Selesai Tugas
              </button>
            </div>
          </div>
        ))}
            ...
            {pendingTasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="font-bold text-gray-800 text-lg group-hover:text-green-600 transition">
                      {task.profiles?.full_name}
                    </h4>
                    <p className="text-xs text-gray-400 mt-1 flex items-center gap-1 bg-gray-50 px-2 py-1 rounded w-fit">
                      <Clock size={12} />{" "}
            ...
            {pendingTasks.length === 0 && (
              <div className="col-span-full py-16 text-center text-gray-400 bg-white border-2 border-dashed border-gray-200 rounded-2xl">
                ...
                <p className="text-sm text-gray-400">
                  Silakan tunggu notifikasi order baru.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            {processTasks.map((task) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                  ...
                  >
                    <MessageCircle size={20} /> Chat App
                  </button>
                  <button
                    onClick={() =>
                      contactUser(
            ...
            {processTasks.length === 0 && (
              <div className="py-16 text-center text-gray-400 bg-white border-2 border-dashed border-gray-200 rounded-2xl">
                ...
                <p className="text-sm text-gray-400">
                  Ambil tugas dari tab Pending untuk mulai bekerja.
                </p>
              </div>
            )}
          </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 animate-in slide-in-from-right overflow-hidden">
      <div className="p-6 border-b border-gray-100 bg-gray-50/50">
        <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2">
          <History className="text-gray-500" /> Riwayat Selesai
        </h3>
      </div>
          ...
          >
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2 md:mb-0 md:justify-start md:gap-4">
                ...
                <span className="md:hidden bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs font-bold">
                  Done
                </span>
              </div>
              <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                <span className="flex items-center gap-1 bg-white border px-2 py-0.5 rounded">
                  <Calendar size={12} />{" "}
              ...
              <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                <MapPin size={12} /> {h.profiles?.address}
              </p>
            </div>
            <div className="text-right flex flex-col items-end min-w-[120px]">
              ...
              <p className="text-xl font-bold text-green-700">
                Rp {h.total_price?.toLocaleString()}
              </p>
              <p className="text-xs text-gray-400">
                {h.waste_types?.name} ({h.weight} Kg)
              </p>
            </div>
          </div>
        ))}
      ...
      <div className="space-y-6 animate-in slide-in-from-right pb-20 md:pb-0">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Tracking komisi dan performa Anda
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-gray-800 mb-1">
              {formatRupiah(earnings.thisWeek)}
            </h3>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-gray-800 mb-1">
              {formatRupiah(earnings.thisMonth)}
            </h3>
          ...
          <div className="bg-gradient-to-br from-amber-400 to-orange-500 p-6 rounded-xl text-white shadow-lg transform hover:scale-105 transition">
            ...
            <p className="text-sm opacity-80">
              {earnings.transactionCount} transaksi
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-800">
            <span className="font-bold">ℹ️ Catatan:</span> Komisi driver adalah
            15% dari setiap transaksi yang selesai. Pembayaran komisi dilakukan
            setiap akhir bulan.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 p-8 text-white text-center">
          <div
            ...
            onClick={() =>
              document.getElementById("driver-avatar-upload").click()
            }
          >
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
              ...
              <p className="text-gray-800 font-medium">
                {driverProfile?.phone || currentUser?.phone || "-"}
              </p>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
              <p className="text-xs text-gray-500 font-bold uppercase mb-1">
              ...
              <p className="text-gray-800 font-medium">
                {driverProfile?.vehicle || "Pickup Truck"}
              </p>
            </div>
          </div>
