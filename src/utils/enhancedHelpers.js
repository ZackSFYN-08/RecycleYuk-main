...
export const getRewardBadge = (points) => {
  if (points >= 5000)
    return { level: "Diamond", color: "bg-purple-500", icon: "💎" };
  if (points >= 2000)
    return { level: "Gold", color: "bg-yellow-500", icon: "🥇" };
  if (points >= 1000)
    return { level: "Silver", color: "bg-gray-400", icon: "🥈" };
  if (points >= 500)
...
export const calculateEnhancedStats = (transactions, userRole) => {
  const stats = {
    ...
    totalWeight: transactions.reduce(
  ...
  stats.trend = {
    ...
    weight:
      recentTransactions.reduce((sum, t) => sum + (Number(t.weight) || 0), 0) -
